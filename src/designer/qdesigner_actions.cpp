      ...
      m_appFontAction(new QAction(tr("Additional Fonts..."), this))
{
    ...
    QDesignerFormWindowManagerInterface *fwManager = m_core->formWindowManager();
    
    // 使用抽象接口直接获取所需的动作和动作组
    m_previewFormAction = fwManager->action(QDesignerFormWindowManagerInterface::DefaultPreviewAction);
    m_styleActions = fwManager->actionGroup(QDesignerFormWindowManagerInterface::StyledPreviewActionGroup);
    m_previewManager = nullptr; // 无法通过抽象接口获取PreviewManager
    
    connect(fwManager, &QDesignerFormWindowManagerInterface::formWindowSettingsChanged,
...
//
    ...
    m_formActions->addAction(m_core->formWindowManager()->action(QDesignerFormWindowManagerInterface::FormWindowSettingsDialogAction));
//
// window actions
//
...
{
    if (m_previewManager && m_previewManager->previewCount()) {
        closePreview();
        return;
    }
